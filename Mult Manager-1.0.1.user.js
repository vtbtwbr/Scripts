...
(function(){
  ...
  function getPlayerName(){
    ...
    // Fallbacks DOM (podem variar por layout/idioma)
    const selectors = [
      '#player_info a',
      '#player_info',
      'a[href*="screen=info_player"][href*="player_id="]',
      '#menu_row2 a[href*="info_player"]',
      'a[href*="screen=info_player"]'
    ];
    for (const sel of selectors){
      const el = document.querySelector(sel);
      if (!el) continue;
      const t = (el.textContent || '').replace(/\s+/g,' ').trim();
      if (t && t.length >= 2 && t.length <= 40 && !/^\d+\|\d+$/.test(t)) return t;
    }

    // Último recurso: procurar algo que pareça "Jogador: Nome"
    try{
      const txt = document.body ? (document.body.textContent || '') : '';
      const m = txt.match(/Jogador\s*[:\-]\s*([A-Za-z0-9_\-\. ]{2,40})/i);
